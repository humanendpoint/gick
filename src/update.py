...
def handle_button_click(request):
    ...
    if "Merge" or "APPROVE" in decision:
        ...
        color = "#ffd500" # yellow
    find_and_update_slack_message(decision_message, pr_title, pr_number, timestamp, color)

def update_slack_message(conf, status, color, timestamp):
    client = WebClient(token=conf.slack_token)
    pr_title = conf.pr_title
    if not update_slack_message_helper(client, conf, status, color, timestamp, pr_title):
        print("Message not found or does not match the criteria.")

def find_and_update_slack_message(decision, compare_pr_title, pr_number, color, timestamp):
    client = WebClient(token=os.environ.get("SLACK_TOKEN"))
    if not find_and_update_slack_message_helper(client, decision, compare_pr_title, pr_number, color, timestamp):
        print("Message not found or does not match the criteria.")
    ...
    for message in messages:
        ...
        for attachment in attachments:
            for block in attachment.get("blocks", []):
                if block.get("type") == "section" and block.get("text", {}).get("text", "") == pr_title:
                    for inner_attachment in reversed(attachments):
                        ...
                        if context_blocks:
                            ...
                            text = last_context_block.get("elements", [])[0].get("text", "").strip()
                            if text == "*Checks*: :processing:":
                                last_context_block["elements"][0]["text"] = f"*Checks*: {status}"
                                inner_attachment["color"] = color
                                updated_attachments = [a if a["id"] == inner_attachment["id"] else a for a in attachments]
                                updated_message = {
                                    "channel": conf.channel_id,
                                    "ts": message["ts"],
                                    "asuser": True,
...
def find_and_update_slack_message_helper(client, decision, compare_pr_title, pr_number, timestamp, color):
    if color is None:
        color = "#0B6623"
    response = client.conversations_history(
    ...
    for message in messages:
        ...
        for attachment in attachments:
            for block in attachment.get("blocks", []):
                if block.get("type") == "section" and compare_pr_title in block.get("text", {}).get("text", ""):
                    for inner_attachment in reversed(attachments):
                        blocks = inner_attachment.get("blocks", [])
                        for block in blocks:
                            if block.get("type") == "actions":
                                ...
                                if last_action_element.get("type") == "button":
                                    text = last_action_element.get("text", {}).get("text", "")
